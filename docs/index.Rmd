...
- `Mutate()` and `summarize()` have can operate on selections of columns with `across()`
...
ds %>% summarize(across(c("height", "mass"), mean_na)) #ROTATE
ds %>% summarize(across(height:mass, mean, na.rm = T)) #ROTATE
ds %>% summarize(across(where(is.numeric), ~ mean(.x, na.rm = T))) #ROTATE
